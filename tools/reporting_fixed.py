    ...
    **kwargs
    ...
    if primary_intents:
        ...
        for i, plan in enumerate(primary_intents, 1):
            ...
            lines.append(f"| {i:2d} | {intent.get('symbol', 'N/A')} | {intent.get('direction', 'N/A')} | ${intent.get('price', 0):.2f} | ${intent.get('stop_price', 0):.2f} | ${intent.get('target_price', 0):.2f} | {intent.get('conviction', 0):.2%} | {intent.get('venue', 'N/A')} | {len(intent.get('evidence_citations', []))} |")
    else:
...
def _ensure_trades_table():
    """Ensure trades table exists in audit DB."""
    AUDIT_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(AUDIT_DB_PATH)
    conn.execute("""
...
def log_trade(
    ...
    **kwargs
    ...
    """
    Log a trade entry or exit to the trades table.
    """
    logger.info(f"Logging trade: {venue} {side} {quantity}x {symbol} @ {price}")
    ...
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO trades (timestamp, venue, symbol, side, quantity, price, order_type, order_id, pnl, status, notes, details)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (timestamp, venue, symbol, side, quantity, price, order_type, order_id, pnl, status, notes, json.dumps(details) if details else None)
    )
    log_id = cursor.lastrowid
    conn.commit()
    conn.close()
    
    return {'log_id': log_id, 'timestamp': timestamp, 'status': 'logged'}


def update_trade_pnl(trade_id: int, pnl: float, status: str = "closed", notes: str = ""):
    """Update a trade with realized P&L on exit."""
    _ensure_trades_table()
    conn = sqlite3.connect(AUDIT_DB_PATH)
    conn.execute(
        "UPDATE trades SET pnl = ?, status = ?, notes = ? WHERE id = ?",
        (pnl, status, notes, trade_id)
    )
    conn.commit()
    conn.close()


def get_todays_trades(venue: str = None) -> List[Dict]:
    ...
    if venue:
        rows = cursor.execute(
            "SELECT * FROM trades WHERE timestamp LIKE ? AND venue = ? ORDER BY timestamp",
            (f"{today}%", venue)
        ).fetchall()
    else:
        rows = cursor.execute(
...
async def get_recent_trades(limit: int = 100, venue: str = None) -> List[Dict]:
    """Get the most recent trades from the audit database."""
    _ensure_trades_table()
    conn = sqlite3.connect(AUDIT_DB_PATH)
    cursor = conn.cursor()
    ...
    if venue:
        rows = cursor.execute(
            "SELECT * FROM trades WHERE venue = ? ORDER BY timestamp DESC LIMIT ?",
            (venue, limit)
        ).fetchall()
    else:
        rows = cursor.execute(
