...
void AuthManager::enter_local_operator() {
    local_operator_ = true;
    ...
    session_.user_info.email = QStringLiteral("local@alphatrader");
    ...
    set_loading(false);
    LOG_INFO("Auth", "Local Alpha Trader operator session — skipping Fincept cloud login");
    emit auth_state_changed();
    emit login_succeeded();
    emit terminal_unlocked();
}
