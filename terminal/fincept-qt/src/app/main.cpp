...
int main(int argc, char* argv[]) {
    ...
    app.setApplicationName("AlphaTrader");
...
#    define FINCEPT_VERSION_STRING "0.0.0-dev"
    ...
    LOG_INFO("App", "Alpha Trader terminal v" FINCEPT_VERSION_STRING " starting...");
    ...
    if (db_result.is_err()) {
        ...
        if (fincept::MigrationRunner::is_fatal_error(db_err)) {
            QMessageBox::critical(nullptr, QObject::tr("Alpha Trader — database error"),
    ...
    fincept::auth::AuthManager::instance().enter_local_operator();
    ...
    // Alpha Trader uses Dexter's Python. Skip Fincept's first-run UV/venv wizard.
    setup_status.needs_setup = false;
    setup_status.needs_package_sync = false;
    ...
    if (setup_status.needs_setup) {
        ...
        setup_screen->setWindowTitle("Alpha Trader — First-Time Setup");
            ...
            setup_screen, &fincept::screens::SetupScreen::setup_complete, [&app, &instance_lock, screen_guard]() {
                ...
                if (!recovered) {
                    ...
                    QTimer::singleShot(1200, &app, [promo_target]() {
                        (void)promo_target; // Alpha Trader: no Fincept Enterprise promo
                    });
                }
