...
QString DockScreenRouter::title_for_id(const QString& id) {
    ...
    static const QHash<QString, const char*> titles = {
        {"alpha_trader", QT_TRANSLATE_NOOP("fincept::DockScreenRouter", "Alpha Trader")},
        {"alpha_chart", QT_TRANSLATE_NOOP("fincept::DockScreenRouter", "Chart")},
...
void DockScreenRouter::attach_group_badge_to_tab(const QString& id, QWidget* screen) {
    ...
    // Screens that default into a desk group (Alpha/CHART/WATCH/…) must pick
    // up the group's current symbol on first materialise, not only when the
    // user clicks the badge.
    if (linked->group() != SymbolGroup::None) {
        const SymbolRef group_sym = SymbolContext::instance().group_symbol(linked->group());
        if (group_sym.is_valid())
            linked->on_group_symbol_changed(group_sym);
        else {
            const SymbolRef own = linked->current_symbol();
            if (own.is_valid())
                SymbolContext::instance().set_group_symbol(linked->group(), own, dynamic_cast<QObject*>(linked));
        }
    }
}
