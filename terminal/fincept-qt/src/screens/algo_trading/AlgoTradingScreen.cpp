...
AlgoTradingScreen::AlgoTradingScreen(QWidget* parent) : QWidget(parent) {
    ...
    connect(&DexterBridge::instance(), &DexterBridge::desk_updated, this, [this](const QJsonObject& payload) {
        if (!dexter_label_)
            return;
        const int hb = payload.value(QStringLiteral("hummingbot_controller_count")).toInt();
        const int vibe = payload.value(QStringLiteral("vibe_skill_count")).toInt();
        const bool ok = payload.value(QStringLiteral("ok")).toBool();
        dexter_label_->setText(ok ? tr("DEXTER  ·  HB %1  ·  VIBE %2  ·  catalog only").arg(hb).arg(vibe)
                                  : tr("DEXTER OFFLINE"));
    });
    if (auto desk = DexterBridge::instance().last_desk(); !desk.isEmpty() && dexter_label_) {
        const int hb = desk.value(QStringLiteral("hummingbot_controller_count")).toInt();
        const int vibe = desk.value(QStringLiteral("vibe_skill_count")).toInt();
        dexter_label_->setText(tr("DEXTER  ·  HB %1  ·  VIBE %2  ·  catalog only").arg(hb).arg(vibe));
    }

    LOG_INFO("AlgoTrading", "Screen constructed");
}

void AlgoTradingScreen::on_group_symbol_changed(const SymbolRef& ref) {
    if (!ref.is_valid())
        return;
    linked_symbol_ = ref.symbol;
    if (status_label_)
        status_label_->setText(tr("SYMBOL %1").arg(ref.symbol));
}

SymbolRef AlgoTradingScreen::current_symbol() const {
    if (linked_symbol_.isEmpty())
        return {};
    return DexterBridge::desk_ref(linked_symbol_);
}
...
QWidget* AlgoTradingScreen::build_status_bar() {
    ...
    hl->addWidget(v1);
    hl->addStretch();
    dexter_label_ = new QLabel(tr("DEXTER …"), bar);
    dexter_label_->setStyleSheet(QString("color:%1; font-size:8px; font-weight:700; font-family:%2;")
                                     .arg(ui::colors::CYAN())
                                     .arg(ui::fonts::DATA_FAMILY));
    hl->addWidget(dexter_label_);
