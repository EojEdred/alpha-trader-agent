...
void PortfolioScreen::build_ui() {
    ...
    dexter_fills_ = new QTableWidget(this);
    dexter_fills_->setColumnCount(6);
    dexter_fills_->setHorizontalHeaderLabels(
        {QStringLiteral("TIME"), QStringLiteral("SYM"), QStringLiteral("SIDE"), QStringLiteral("SRC"),
         QStringLiteral("ENTRY"), QStringLiteral("PNL")});
    dexter_fills_->verticalHeader()->setVisible(false);
    dexter_fills_->setEditTriggers(QAbstractItemView::NoEditTriggers);
    dexter_fills_->setSelectionBehavior(QAbstractItemView::SelectRows);
    dexter_fills_->horizontalHeader()->setStretchLastSection(true);
    dexter_fills_->setFixedHeight(110);
    dexter_fills_->setStyleSheet(QString("QTableWidget { background:%1; color:%2; gridline-color:%3; "
                                         "font-family:'Consolas','Courier New',monospace; font-size:11px; }"
                                         "QHeaderView::section { background:%4; color:%5; border:0; padding:3px; }")
                                     .arg(ui::colors::BG_SURFACE(), ui::colors::TEXT_PRIMARY(), ui::colors::BORDER_DIM(),
                                          ui::colors::BG_RAISED(), ui::colors::TEXT_SECONDARY()));
    layout->addWidget(dexter_fills_);
    connect(dexter_fills_, &QTableWidget::cellClicked, this, [this](int row, int) {
        if (auto* it = dexter_fills_->item(row, 1))
            on_symbol_selected(it->text());
    });
    connect(&DexterBridge::instance(), &DexterBridge::desk_updated, this, [this](const QJsonObject& payload) {
        render_dexter_fills(payload.value(QStringLiteral("paper_trades")).toArray());
    });
    render_dexter_fills(DexterBridge::instance().last_desk().value(QStringLiteral("paper_trades")).toArray());
...
void PortfolioScreen::render_dexter_fills(const QJsonArray& rows) {
    if (!dexter_fills_)
        return;
    dexter_fills_->setRowCount(rows.size());
    for (int i = 0; i < rows.size(); ++i) {
        const QJsonObject o = rows.at(i).toObject();
        const QString ts = o.value(QStringLiteral("timestamp")).toString();
        const QString time = ts.length() >= 19 ? ts.mid(11, 8) : ts;
        auto put = [&](int col, const QString& text) {
            dexter_fills_->setItem(i, col, new QTableWidgetItem(text));
        };
        put(0, time);
        put(1, o.value(QStringLiteral("symbol")).toString());
        put(2, o.value(QStringLiteral("direction")).toString());
        put(3, o.value(QStringLiteral("source")).toString());
        put(4, o.value(QStringLiteral("entry")).toString());
        put(5, o.value(QStringLiteral("pnl")).toString());
    }
    dexter_fills_->resizeColumnsToContents();
}
