...
NewsScreen::NewsScreen(QWidget* parent) : QWidget(parent) {
    ...
    auto apply_dexter_news = [this](const QVector<fincept::services::NewsArticle>& articles) {
        if (articles.isEmpty())
            return;
        QSet<QString> have;
        for (const auto& existing : all_articles_)
            have.insert(existing.id);
        int added = 0;
        for (const auto& a : articles) {
            if (a.headline.isEmpty() || have.contains(a.id))
                continue;
            all_articles_.prepend(a);
            have.insert(a.id);
            ++added;
        }
        if (added == 0)
        ...
        if (search_query_.isEmpty())
            search_query_ = QStringLiteral("SPY");
        apply_filters_async();
    };
    connect(&DexterBridge::instance(), &DexterBridge::news_updated, this, apply_dexter_news);
    apply_dexter_news(DexterBridge::instance().last_news());
