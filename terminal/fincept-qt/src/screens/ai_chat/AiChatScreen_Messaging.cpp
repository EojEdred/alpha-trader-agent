...
void AiChatScreen::on_send() {
    ...
    const bool dexter_brain = fincept::auth::AuthManager::instance().is_local_operator();
    if (!dexter_brain && !ai_chat::LlmService::instance().is_configured()) {
    ...
    const QString req_provider = dexter_brain ? QStringLiteral("dexter")
                                             : ai_chat::LlmService::instance().active_provider();
    const QString req_model = dexter_brain ? QStringLiteral("cli")
    ...
    total_messages_++;
    ChatRepository::instance().add_message(active_session_id_, "user", text, req_provider, req_model);
...
void AiChatScreen::send_via_dexter(const QString& text) {
    QJsonObject body;
    body["message"] = text;
    QPointer<AiChatScreen> self = this;
    HttpClient::instance().post(
        ...
        [self](fincept::Result<QJsonDocument> result) {
            if (!self)
                return;
            ai_chat::LlmResponse resp;
            if (result.is_err()) {
                resp.success = false;
                resp.error = QString::fromStdString(result.error());
            } else {
                resp.success = true;
                resp.content = result.value().object().value("text").toString();
            }
            self->on_streaming_done(resp);
        },
        this);
