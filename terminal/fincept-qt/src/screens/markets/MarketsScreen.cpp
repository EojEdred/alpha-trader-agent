...
void MarketsScreen::wire_panel(MarketPanel* p) {
    ...
    connect(p, &MarketPanel::symbol_selected, this, [this](const QString& sym) {
        last_symbol_ = sym;
        if (link_group_ != SymbolGroup::None)
            SymbolContext::instance().set_group_symbol(link_group_, SymbolRef::equity(sym, QStringLiteral("US")), this);
    });
}
...
void MarketsScreen::on_group_symbol_changed(const SymbolRef& ref) {
    if (!ref.is_valid())
        return;
    last_symbol_ = ref.symbol;
}

SymbolRef MarketsScreen::current_symbol() const {
    if (last_symbol_.isEmpty())
        return {};
    return SymbolRef::equity(last_symbol_, QStringLiteral("US"));
}
