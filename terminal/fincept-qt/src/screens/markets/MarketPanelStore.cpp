...
QVector<MarketPanelConfig> MarketPanelStore::load() {
    ...
    for (const auto& p : panels) {
        if (p.title.compare(QLatin1String("Alpha Desk"), Qt::CaseInsensitive) == 0) {
            has_alpha = true;
            break;
        }
    }
    if (!has_alpha && !g_panel_load_failed) {
        auto alpha = MarketPanelConfig::make(
            QStringLiteral("Alpha Desk"),
            {"SPY", "QQQ", "AAPL", "NVDA", "MSFT", "AMZN", "META", "TSLA", "GLD", "TLT"}, false);
        alpha.column_index = 0;
        alpha.splitter_index = 0;
        panels.prepend(alpha);
        save(panels);
    }
...
QVector<MarketPanelConfig> MarketPanelStore::build_defaults() {
    ...
    auto alpha = MarketPanelConfig::make(QStringLiteral("Alpha Desk"),
                                         {"SPY", "QQQ", "AAPL", "NVDA", "MSFT", "AMZN", "META", "TSLA", "GLD", "TLT"},
                                         false);
    alpha.column_index = 0;
    panels.append(alpha);
